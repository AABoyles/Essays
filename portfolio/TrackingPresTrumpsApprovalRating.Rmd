...
For another significant refinement to this conception of the distribution of approval ratings, day-to-day ratings seem to be correlated. For example, here are [Barack Obama's approval ratings](http://www.gallup.com/interactives/185273/presidential-job-approval-center.aspx) over his two terms:
...
OK, since we only drop into the lower band thrice, let's say that lower-band probability is something like `r round(3/n, 2)` and the middle-band probability is `r round(53/n, 2)`, and the high-band probability is very small.
...
So, on average, the Prophet estimate has been off by about `r round(MAE, 2)` points. It's interesting to note that this is within Gallup's own 3-point margin of error, so refining these predictions for higher accuracy doesn't make a lot of sense.
...
At this point, it looks like modeling approval and disapproval separately actually is the correct way to go, increasing the fit of the model. Ordinarily, I'd test this with some cross-validation procedure(s), but we're still working with so little data that cross-validation is difficult.
...
![](http://anthony.boyles.cc/imgs/Screenshot_2017-03-16_10-49-06.png)
...
...[isn't good for the President](http://www.vox.com/world/2017/3/20/14982018/comey-fbi-investigation-russia-trump-campaign).
