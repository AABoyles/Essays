...
OK, so let's say that lower-band probability is something like `r round(temp[1,2]/n, 2)` and the middle-band probability is `r round(temp[2,2]/n, 2)`, and the high-band probability is very small.
...
This highly confident low-band prediction *feels* wrong to me, which I take to be an important signal. GJO consensus remains that the middle band is likeliest, though the recent run of low-band days has driven the two to begin collapsing toward one another:

![](http://anthony.boyles.cc/imgs/Screenshot_2017-04-04_13-44-43.png)

Based on these two conflicting (at the margin) patterns, I'm going to update in roughly equal measure and ultimately stand by my prior estimate of Low: 80%, Mid: 19%, High: 1%.