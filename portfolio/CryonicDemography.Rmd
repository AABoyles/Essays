...
It appears that Cryonics is unlikely to experience explosive growth in the next 20 years. Obviously, this discounts any substantive events, like a successful demonstration of a complete revival of a cryopreserved large animal ([rabbit](http://www.brainpreservation.org/asc_rabbit_fulleval/) or dog seem the likeliest cases).
...
cryonauts %>%
  dplyr::filter(!is.na(Deanimation), !is.na(Type)) %>%
  group_by(Type) %>%
  dplyr::mutate(
    c = 1,
    cumulation = cumsum(c)
  ) %>%
  plot_ly(x=~Deanimation, y=~cumulation, type='scatter', mode='lines', color=~Type) %>%
  layout(
    xaxis = list(title = "Date"),
    yaxis = list(title = "Number of Cryonauts")
  )
