...
predictions %>%
  select(-TextTargetDate, -Adjustment:-Canceled, -Source:-Description) %>%
  datatable()
```
...
predictions %>%
  filter(
    ...
    !is.na(ActualLength)) %>%
  plot_ly() %>%
    add_markers(x=~PredictionLength, y=~ActualLength, text=~Prediction, hoverinfo="text", name="Actual Predictions") %>%
