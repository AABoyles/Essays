...
monthly <- cryonauts %>%
  ...
  ungroup() %>%
  right_join(yms) %>%
  filter(!(Year >= year(now()) & Month >= month(now()))) %>%
  mutate(
...
```{r, warning=FALSE}
cryonauts %>%
  filter(!is.na(Deanimation), !is.na(Gender)) %>%
  group_by(Gender) %>%
  mutate(
    c = 1,
    cumulation = cumsum(c)
  ) %>%
  plot_ly(x=~Deanimation, y=~cumulation, type='scatter', mode='lines', color=~Gender) %>%
  layout(
...
```{r, warning=FALSE}
cryonauts %>%
  filter(!is.na(Deanimation), !is.na(Type)) %>%
  group_by(Type) %>%
  mutate(
    c = 1,
    cumulation = cumsum(c)
...
cryonauts %>%
  filter(!is.na(Deanimation), Status != "Preserved") %>%
  mutate(
    c = 1,
    cumulation = cumsum(c)
  ) %>%
