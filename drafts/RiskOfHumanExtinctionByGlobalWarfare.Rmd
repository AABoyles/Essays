...
source("../infobox.R")
load_package("HistData")
load_package("readr")
load_package("readxl")
load_package("magrittr")
load_package("dplyr")
load_package("tidyr")
load_package("fuzzyjoin")
load_package("intubate")
load_package("plotly")
load_package("ShRoud")
...
[Importance, Tractibility, and Neglectedness](https://concepts.effectivealtruism.org/concepts/importance-neglectedness-tractability/)

### Importance

At any given time, the most important existential risk is that which is going to occur next. For example, suppose an observer 66 million years ago wished to embark upon a project to protect its species from existential risks. Many of the hazards we identify as potential existential risks today would be the same for our 65-million year old observer. For example, the greenhouse effect might threaten the ecosystem upon which the observer relies for survival, and so the observer might conclude that a strategy of greenhouse effect mitigation would be the most effecacious approach to reducing its existential risk. However, soon thereafter the [Chicxulub impactor](https://en.wikipedia.org/wiki/Chicxulub_impactor) struck the earth and destroyed the ecosystem anyway.

The observer probably couldn't have known that the asteroid was a greater threat than global climate change. Because we cannot forecast most risks over the short term with any degree of certainty, we must instead estimate their probabilities densities over time, and at any given time work to mitigate that one which has the highest probability. Accordingly, the primary goal of this work is to estimate the probability of existential risky warfare. As a secondary goal, this should give us useful insights into the second-order effects of wars on society which are likely to affect humanity's ability to mitigate other existential risks.

### Tractibility

If $R_a$ will occur in the next century with probability $0.5$, and $R_b$ will occur over the same time horizon with probability $0.1$, then work on $R_a$ will be roughly five times as important as work on $R_b$. However, if $R_a$ is intractible (for example, an unpredictable destructive solar phenoenon), then work on $R_b$ is infinitely more important than work on $R_a$.

### Neglectedness

If $R_a$ and $R_b$ are both tractible, then the final criterion of consideration should be neglectedness. If $R_a$ is five times as important as $R_b$, does it benefit from five times as much public attention? As a rough heuristic, 
...
if(!file.exists("../data-cache/ConflictCatalog18vars.xls")){
 ...
 stop('Needed file not found. Please open ../data-cache/ConflictCatalog18vars.xls and save it as a CSV')
}
ConflictCatalog <- read_csv("../data-cache/ConflictCatalog18vars.csv")
```
...
```{r populationData, message=FALSE, eval=FALSE}
if(!file.exists("../data-cache/PopEstimates.csv")){
 #TODO: Throw an Error
 stop('Need Population Estimates Data.')
}

pop <- read_csv("../data-cache/PopEstimates.csv") %>%
...
[Backing into World War III](https://www.brookings.edu/research/backing-into-world-war-iii/)
...
We observe that the Cold War between the United States and the Soviet Union did not culminate in a nuclear war. The anthropic logic here is that an observer is vastly more likely to live in a world where catastrophic nuclear war has not occurred, as such a war would vastly reduce the number of observers capable of making the observation.

Note that this says nothing about the prior probability of that war, other than that it was less than 1\. So, what _does_ this tell us? More or less nothing, I’m afraid. As [Roko Jelavic points out](https://www.facebook.com/rjelavic/posts/10211689902182034?match=cm9rbyBqZWxhdmnEhyxhbnRocm9waWMsamVsYXZpxIcscm9rbw%3D%3D), we shouldn't use our empirical observations about existential risks as a basis for reasoning about them:
...
Fixing a probability on existentially risky warfare is difficult. For a variety of reasons, the simple "outside view" breaks down at Wars much too small to be existentially risky. We can confidently say that the probability is considerably higher than the outside view predicts. Finally, anthropic reasoning shows that we should not be surprised that an extinctive war has not ocurred, and this should not comfort us.
