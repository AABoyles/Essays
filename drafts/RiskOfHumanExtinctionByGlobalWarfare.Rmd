...
One of the most robust empirical properties of conflict (of which we are presently aware) is the 
...
Quarrels %>%
  group_by(logDeaths) %>%
  dplyr::summarise(numWars = n()) %>%
  arrange(desc(logDeaths)) %>%
  mutate(logLargerWars = 7*max0(log10(cumsum(numWars)))) %>%
  plot_ly(x=logDeaths, y=logLargerWars, type="point", mode="markers")
```
...
wars <- as.data.frame(interstatewars) %>%
  ...
  plot_ly(x = logDeaths, y = logRank,
    text = paste0(WarName, " (", StartYear, "-", EndYear, ")<br />", prettyNum(TotalCombatDeaths, big.mark = ","), " casualties"),
